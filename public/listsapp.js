...
async function insert() {
  if (input.value !== "") {
    const res = await fetch(`${url}/newtodo/${input.value}`, {
      mode: "no-cors",
    });
...
randamadd.addEventListener("click", async () => {
  ...
  const res = await fetch(`${url}/newtodo/${fbored.joke}`, { mode: "no-cors" });
  const nb = document.createElement("li");
  nb.classList.add("data");
...
ol.addEventListener("click", async (e) => {
  const res = await fetch(`${url}/${e.target.innerText}`, { mode: "no-cors" });
